...
// to remove (used currently to prevent duplicate
// of WCBoardsDidChangeUnreadCountNotification symbol)
#import "WCBoards.h"
...
@interface WCBoardsViewController (Private)

- (id)_initBoardsControllerWithConnection:(WCServerConnection *)connection;
...
@implementation WCBoardsViewController (Private)
...
- (id)_initBoardsControllerWithConnection:(WCServerConnection *)connection {
    ...
    self = [super initWithNibName:@"BoardsView" bundle:nil];
...
//        thread    = [self _threadAtIndex:index];
//        
//        if(thread) {
...
@implementation WCBoardsViewController

#pragma mark -

+ (id)boardsControllerWithConnection:(WCServerConnection *)connection {
    return [[[[self class] alloc] _initBoardsControllerWithConnection:connection] autorelease];
...
//    NSToolbar			*toolbar;
	...
	_errorQueue = [[WCErrorQueue alloc] initWithWindow:[[self view] window]];
...
- (void)wiredBoardGetBoardsReply:(WIP7Message *)message {
    ...
    NSLog(@"wiredBoardGetBoardsReply: %@", message);
...
- (BOOL)showNextUnreadThread {
	...
	if(thread) {
		[[[self view] window] makeFirstResponder:_threadsTableView];
...
- (id)outlineView:(NSOutlineView *)outlineView objectValueForTableColumn:(NSTableColumn *)tableColumn byItem:(id)item {
	...
	if(tableColumn == _boardTableColumn) {
		...
		return [NSImage imageWithPillForCount:count
							   inActiveWindow:([NSApp keyWindow] == [[self view] window])
